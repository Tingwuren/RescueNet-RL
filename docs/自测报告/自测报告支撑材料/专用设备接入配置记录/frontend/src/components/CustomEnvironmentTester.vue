      ...
      <section class="tester__section">
        ...
        <div class="devices">
          <div class="devices__header">
            <div>
              <p>残余基站</p>
              <small>不添加则按无残余基站测试；添加后默认放置在用户密集区域。</small>
            </div>
          ...
          <p v-if="!baseStations.length" class="hint">尚未添加残余基站，本次测试将使用无残余基站初始状态。</p>
        </div>
...
const currentSceneSignature = computed(() =>
  JSON.stringify({
    ...
    protocol: evaluationProtocol.value,
...
const clampGridIndex = (value, max, fallback = 0) => {
  const numeric = Number(value);
  const next = Number.isFinite(numeric) ? Math.round(numeric) : fallback;
  return Math.max(0, Math.min(Math.max(0, max), next));
};

const defaultResidualBasePosition = () => {
  const scenario = currentScenario.value;
  const fallback = {
    x: Math.round(gridRowLimit.value / 2),
    y: Math.round(gridColLimit.value / 2),
  };
  const clusters = Array.isArray(scenario?.user_clusters) ? scenario.user_clusters : [];
  let bestCluster = null;
  let bestScore = -Infinity;
  clusters.forEach((cluster) => {
    if (!Array.isArray(cluster?.center) || cluster.center.length < 2) return;
    const density = Number(cluster.density ?? 0);
    const demand = Number(cluster.demand_mbps ?? 1);
    const score = density * Math.max(1, demand);
    if (score > bestScore) {
      bestScore = score;
      bestCluster = cluster;
    }
  ...
  if (bestCluster) {
    return {
      x: clampGridIndex(bestCluster.center[0], gridRowLimit.value, fallback.x),
      y: clampGridIndex(bestCluster.center[1], gridColLimit.value, fallback.y),
    };
  }

  const preview = Array.isArray(scenario?.candidate_site_preview) ? scenario.candidate_site_preview : [];
  const candidate = preview.find((site) => site.category === "\u6838\u5fc3\u8986\u76d6") || preview[0];
  if (candidate) {
    return {
      x: clampGridIndex(candidate.x, gridRowLimit.value, fallback.x),
      y: clampGridIndex(candidate.y, gridColLimit.value, fallback.y),
    };
  }
...
const addBaseStation = () => {
  ...
  const position = defaultResidualBasePosition();
...
const buildCustomBaseStationsPayload = () =>
  baseStations.value
    .map((station) => {
      if (!station?.base_station) return null;
      const mode = station.mode || resolveDefaultMode(station.base_station);
      if (!mode) return null;
      return {
        base_station: station.base_station,
        mode,
        x: clampGridIndex(station.x, gridRowLimit.value),
        y: clampGridIndex(station.y, gridColLimit.value),
      };
    })
    .filter(Boolean);
...
const runSimulation = async () => {
  ...
  terminalStatus.value = "running";
  sceneGraphTab.value = "imported";
  appendTerminalLine(`使用导入场景快照启动测试：${scenarioName.value}，协议=${evaluationProtocol.value}。`);
