...
class MultiModalCommEnv(gym.Env):
    ...
    def __init__(
        ...
        stress_profile: Optional[Dict[str, Any]] = None,
        ...
        self.stress_profile: Dict[str, Any] = dict(stress_profile or {})
        self.evaluation_protocol = str(
            self.stress_profile.get("name") or self.stress_profile.get("protocol") or "standard"
        )
        self.residual_fraction = float(np.clip(self._stress_float("residual_fraction", 0.25), 0.0, 1.0))
        self.demand_multiplier = max(0.1, self._stress_float("demand_multiplier", 1.0))
        self.coverage_radius_multiplier = max(0.1, self._stress_float("coverage_radius_multiplier", 1.0))
        self.capacity_multiplier = max(0.0, self._stress_float("capacity_multiplier", 1.0))
        self.availability_multiplier = max(0.0, self._stress_float("availability_multiplier", 1.0))
        self.broadcast_bandwidth_multiplier = max(
            0.0,
            self._stress_float("broadcast_bandwidth_multiplier", self.capacity_multiplier),
        )
        self.broadcast_coverage_multiplier = max(0.0, self._stress_float("broadcast_coverage_multiplier", 1.0))
        ...
        self.max_steps = int(max_steps_override) if max_steps_override else self.scenario.max_steps
    ...
    def _stress_float(self, key: str, default: float) -> float:
        try:
            return float(self.stress_profile.get(key, default))
    ...
    def _sample_users(self) -> None:
        ...
        if not clusters:
            ...
            self._apply_user_demand_stress()
            return
    ...
    def _apply_user_demand_stress(self) -> None:
        if abs(self.demand_multiplier - 1.0) <= 1e-6:
            return
        upper = max(40.0, 40.0 * self.demand_multiplier)
        self.user_demands = np.clip(self.user_demands * self.demand_multiplier, 2.0, upper).astype(np.float32)
    ...
    def _get_time_snapshot(self) -> Tuple[Dict[str, float], Dict[str, float]]:
        ...
        for mode in self.communication_modes:
            metric = record.mode_metrics.get(mode)
            if metric:
                available_bandwidth = metric.available_bandwidth * self.capacity_multiplier
                availability = np.clip(metric.availability * self.availability_multiplier, 0.0, 1.0)
            else:
                available_bandwidth = 0.0
                availability = 0.0
            mode_snapshot[mode] = (float(available_bandwidth), float(availability))

        broadcast_snapshot = {}
        for mode in self.broadcast_modes:
            metric = record.broadcast_metrics.get(mode)
            if metric:
                available_bandwidth = metric.available_bandwidth * self.broadcast_bandwidth_multiplier
                coverage = np.clip(metric.coverage * self.broadcast_coverage_multiplier, 0.0, 1.0)
            else:
                available_bandwidth = 0.0
                coverage = 0.0
            broadcast_snapshot[mode] = (float(available_bandwidth), float(coverage))
    ...
    def reset(
        ...
        *,
        ...
        if self.custom_base_station_specs is not None:
            ...
            num_residual = int(self.num_users * self.residual_fraction)
    ...
    def _deploy_comm(self, site_idx: int, comm_idx: int) -> Dict[str, float]:
        ...
        coverage_radius = float(profile.get("coverage_radius", 3.0)) * self.coverage_radius_multiplier
    ...
    def _info_dict(self) -> Dict[str, Any]:
        return {
            ...
            "has_residual_network": self._has_active_residual_network(),
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        scalars = np.array(
            [
                float(self._has_active_residual_network()),
